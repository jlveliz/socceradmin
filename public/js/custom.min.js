$(function() {
            ...
            .done((data) => {
                ...
                if (data.length > 0) {
                    data.forEach((item, key) => {
                        ...
                        htmlData += "<td> <button resource='" + JSON.stringify(item) + "' class='btn btn-secondary btn-sm btn-flat close-search-modal' data-dismiss='modal'><i class='i-Cursor-Click'></i></button></td>";
                        htmlData += "<tr>";
                    })
                } else {
    ...
    */
    ...
    /*
        sidebar left
    */
    $('.sidebar-left-secondary').mouseleave(function(event) {
...
function get_group_names(keyGroup) {
    // debugger
    var groups = [
        { 'little-toes': 'Little Toes' },
        { 'big-toes': 'Big Toes' },
        { 'happyfeet': 'HappyFeet' },
        { 'happyfeet-avanzado': 'HappyFeet Avanzado' }
    ];

    var found = false;

    for (var i = 0; i < groups.length; i++) {
        if (groups[i][keyGroup]) {
            found = groups[i][keyGroup];
        }
    }
